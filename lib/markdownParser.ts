 ...
 * @description Utilitário para converter texto Markdown em HTML com classes Tailwind personalizadas.
 */

/**
 * Converte uma string Markdown para HTML estilizado.
 *
 * Suporta:
 * - Títulos (#, ##, ###)
 * - Negrito (**texto**)
 ...
 * - Quebra de linha
 *
 * @param markdown - A string contendo o Markdown a ser convertido.
 * @returns A string HTML convertida e estilizada.
 *
 * @example
 * ```typescript
...
export const parseMarkdown = (markdown: string): string => {
  ...
  html = html.replace(
    /^## (.*$)/gim,
    "<h2 class='text-2xl font-semibold mt-8 mb-4 text-foreground'>$1</h2>"
  );
  html = html.replace(
    /^# (.*$)/gim,
    "<h1 class='text-3xl font-bold mt-8 mb-5 text-foreground'>$1</h1>"
  );
